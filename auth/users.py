...
Passwords are always bcrypt-hashed at rest. If this app is upgraded
from the old plaintext-password version, any legacy plaintext
passwords found in users.json are transparently migrated to bcrypt
hashes the first time the file is loaded (and the file is rewritten).

Roles: "Admin", "Manager", "User" (RBAC - see security/rbac.py for
what each role is allowed to do). Role is only ever read from this
server-side store -- never trust a role value coming from the client.
"""
...
def _default_users():
    """
    Seeds the initial account(s) on first run:
    - Always seeds one Admin account (BOOTSTRAP_ADMIN_USERNAME/PASSWORD).
    - Optionally seeds a second account (BOOTSTRAP_USER_USERNAME/PASSWORD/
      ROLE) if BOOTSTRAP_USER_PASSWORD is set -- useful for a standing
      "User" or "Manager" login that needs to survive a fresh/ephemeral
      deployment (e.g. Streamlit Community Cloud, where the filesystem is
      reset on reboot/redeploy) without an admin having to recreate it by
      hand every time.
    If BOOTSTRAP_ADMIN_PASSWORD is not set in the environment, a random
    admin password is generated and printed to the server console ONCE so
    an operator can capture it -- the app never ships with a hard-coded
    ...
    if BOOTSTRAP_USER_PASSWORD:
        role = BOOTSTRAP_USER_ROLE if BOOTSTRAP_USER_ROLE in VALID_ROLES else "User"
        users[BOOTSTRAP_USER_USERNAME] = {
            "password": hash_password(BOOTSTRAP_USER_PASSWORD),
            "role": role,
        }
